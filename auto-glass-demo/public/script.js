...
menu?.addEventListener('click',()=>setMenu(!nav.classList.contains('open')));backdrop?.addEventListener('click',()=>setMenu(false));$$('#nav a').forEach(a=>a.addEventListener('click',()=>setMenu(false)));window.addEventListener('resize',()=>{if(innerWidth>900)setMenu(false)});

// Quote wizard
const form=$('#quoteForm');
if(form){let step=1;const steps=$$('.form-step'),labels=$$('.steps span'),bar=$('#progress'),stepCount=$('#stepCount'),success=$('#success');
  const show=n=>{step=n;steps.forEach((el,i)=>el.classList.toggle('active',i===n-1));labels.forEach((el,i)=>el.classList.toggle('active',i===n-1));bar.style.width=`${n/3*100}%`;stepCount.textContent=`Step ${n} of 3`;form.scrollIntoView({behavior:'smooth',block:'center'});};
  ...
  const photo=$('#photo'),photoText=$('#photoText'),photoStatus=$('#photoStatus');photo?.addEventListener('change',()=>{const f=photo.files[0];photoText.textContent=f?f.name:'Add a damage photo';photoStatus.textContent=f?'Selected ✓':'Not selected';photoStatus.classList.toggle('selected',!!f)});
  ...
  const phone=$('#phoneInput');phone?.addEventListener('input',()=>{const d=phone.value.replace(/\D/g,'').slice(0,10);phone.value=d.length>6?`${d.slice(0,3)}-${d.slice(3,6)}-${d.slice(6)}`:d.length>3?`${d.slice(0,3)}-${d.slice(3)}`:d});
  form.addEventListener('submit',e=>{e.preventDefault();if(!valid())return;steps.forEach(x=>x.classList.remove('active'));success.classList.add('show');bar.style.width='100%';stepCount.textContent='Complete';success.focus?.()});
  $('#reset')?.addEventListener('click',()=>{form.reset();success.classList.remove('show');photoText.textContent='Add a damage photo';photoStatus.textContent='Not selected';photoStatus.classList.remove('selected');show(1)});
}
...
function renderDiag(){if(!diagQuestion)return;const d=diagQuestions[diagStep];diagProgress.textContent=`Question ${diagStep+1} of ${diagQuestions.length}`;diagBar.style.width=`${(diagStep/diagQuestions.length)*100}%`;diagQuestion.innerHTML=`<h3>${d.q}</h3><div class="diag-options">${d.opts.map((o,i)=>`<button type="button" data-diag="${i}">${o[0]}<span>→</span></button>`).join('')}</div><small>Educational screening only — not a repair guarantee.</small>`;$$('[data-diag]',diagQuestion).forEach(btn=>btn.addEventListener('click',()=>{const opt=d.opts[Number(btn.dataset.diag)];diagScore+=opt[1];diagFlags.push(opt[2]);diagStep++;if(diagStep<diagQuestions.length)renderDiag();else showDiagResult()}));}
function showDiagResult(){diagQuestion.hidden=true;diagResult.hidden=false;diagBar.style.width='100%';diagProgress.textContent='Screening complete';let title,text;if(diagScore<=1){title='This may be a repair candidate.';text='The answers are relatively favourable for a repair assessment. Size, depth, exact location and contamination still need a technician’s review.'}else if(diagScore<=4){title='A technician should inspect it.';text='One or more factors can affect repair suitability. Send a photo and vehicle details so the shop can advise the right next step.'}else{title='Replacement may be more likely.';text='The answers include factors often associated with replacement, but the website should never make the final technical decision without inspection.'}$('#resultTitle').textContent=title;$('#resultText').textContent=text;$('#resultFlags').innerHTML=diagFlags.slice(-3).map(f=>`<span>${f}</span>`).join('')}
